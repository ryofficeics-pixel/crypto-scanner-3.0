...
const IMPULSE_THRESHOLD = 0.012; // 1.2% body move qualifies as "strong" impulse
...
export function findOrderBlocks(candles: Candle[]): OrderBlock[] {
  ...
  for (let i = 1; i < candles.length - 1; i++) {
    ...
    if (cur.close < cur.open && bodyMove >= IMPULSE_THRESHOLD) {
      blocks.push({ index: i, type: "bullish", top: cur.high, bottom: cur.low, time: cur.closeTime, mitigated: false });
    }
    if (cur.close > cur.open && bodyMove <= -IMPULSE_THRESHOLD) {
      blocks.push({ index: i, type: "bearish", top: cur.high, bottom: cur.low, time: cur.closeTime, mitigated: false });
    }
  }
...
export function findLiquidityZones(pivots: Pivot[], tolerancePct = 0.15): LiquidityZone[] {
